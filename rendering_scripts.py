...
def generate_interpolation_video(run_id, snapshot=None, grid_size=[1,1], image_shrink=1, image_zoom=1, duration_sec=60.0, smoothing_sec=1.0, mp4=None, mp4_fps=30, mp4_codec='libx265', mp4_bitrate='16M', random_seed=1000, minibatch_size=8):
    ...
    random_state = np.random.RandomState(random_seed)
    if mp4 is None:
        mp4 = misc.get_id_string_for_network_pkl(network_pkl) + '-seed-' + str(random_seed) + '.mp4'
    ...
    result_subdir = "results/videos"
    moviepy.editor.VideoClip(make_frame, duration=duration_sec).write_videofile(os.path.join(result_subdir, mp4), fps=mp4_fps, codec='mpeg4', bitrate=mp4_bitrate)
...
if __name__ == "__main__":
    ...
    tfutil.init_tf()
    generate_interpolation_video(9, grid_size=[1,1], random_seed=int(time.time()), mp4_fps=75, duration_sec=180.0)
    print('Exiting...')
